...
void ofApp::setup(){
   ...
   paletteView = 0;
    ...
    ofAddListener(ColourLoveEvent::events, this, &ofApp::colourLoveEvent);
...
void ofApp::colourLoveEvent(ColourLoveEvent &e) {
    palettes = e.palettes;
...
void ofApp::updateColourLab(){
    
    if(colourLab){
         ofRemoveListener(colourLab->newGUIEvent, this, &ofApp::colourLabEvent);
    ...
    //colourLab->autoSizeToFitWidgets();
    //
    ofAddListener(colourLab->newGUIEvent, this, &ofApp::colourLabEvent);
...
void ofApp::colourLabEvent(ofxUIEventArgs &e){
    //if(mouseDown){
...
void ofApp::setPalette(int pId){
    ...
    if(paletteView){
        ofRemoveListener(paletteView->newGUIEvent, this, &ofApp::colourPaletteEvent);
        delete paletteView;
        paletteView = 0;
    }
    paletteView  = new ofxUICanvas(palettesX, 0, ofGetWidth()-palettesX, ofGetHeight());
    ofAddListener(paletteView->newGUIEvent, this, &ofApp::colourPaletteEvent);
...
void ofApp::colourPaletteEvent(ofxUIEventArgs &e){
    string name = e.widget->getName(); 
	int kind = e.widget->getKind(); 
    int uid = e.widget->getID();
